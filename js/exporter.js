...
export function generateHTMLExport(exportMode, projectName, classes, rels, nodes, imgData) {
  ...
  `:`
    ...
    .copyright { position: fixed; bottom: 10px; left: 15px; font-size: 0.7rem; color: #718096; background: rgba(255,255,255,0.8); padding: 4px 8px; border-radius: 4px; z-index: 2000; pointer-events: none; }
  `;
...
export async function copyCode(files, codes, toastFn) {
  try {
    await navigator.clipboard.writeText(files.map((f, i) => `// ===== ${f.name} =====\n${codes[i]}`).join('\n\n'));
    toastFn('✅ הועתק');
  } catch {
...
export async function exportPDF(nodes, classes, rels, files, CW, ACOL, CCOL, drawCardFn, toastFn) {
  ...
  try{
    ...
    // ── Cover page: Circular / Flow layout ──
    {
      ...
      // --- ציור זכויות יוצרים בצד ימין למטה של עמוד השער ---
      cctx.textAlign = 'right';
      cctx.fillStyle = 'rgba(255,255,255,0.6)';
      cctx.fillText('© כל הזכויות שמורות ל-Shmuel Buchnick | shmuel1.618@gmail.com', LW - 32, ly2);
      cctx.textAlign = 'start';
    ...
    for(let p=0;p<totalPages;p++){
      ...
      pageCards.forEach(pn=>{
        ...
        drawCardFn(pctx, fake, true, false, false);
      });
    ...
    toastFn(`✅ PDF נשמר — ${totalPages} עמודים`);
  } catch(e){
    console.error(e); toastFn('❌ שגיאה: '+e.message);
  }
